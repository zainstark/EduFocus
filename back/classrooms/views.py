...
from .serializers import (
    ...
    PerformanceSerializer, DashboardSessionSerializer,
    StudentPerformanceSerializer, ClassroomStatsSerializer,
    StudentClassStatsSerializer
)
from .permissions import IsInstructor, IsStudent, IsClassroomInstructor, IsEnrolledStudent
...
class InstructorDashboardAPI(APIView):
    ...
    def get(self, request):
        ...
        serializer = DashboardSessionSerializer(recent_sessions, many=True)
...
class StudentDashboardAPI(APIView):
    ...
    def get(self, request):
        ...
        ).select_related('session', 'session__classroom').order_by('-session__start_time')[:10]
        
        return Response({
            'full_name': request.user.full_name,
            'live_classes': ClassroomSerializer(live_classes, many=True).data,
            'enrolled_classes': ClassroomSerializer(enrolled_classes, many=True).data,
            'session_history': StudentPerformanceSerializer(session_history, many=True).data
        })
...
class JoinClassroomAPI(APIView):
    permission_classes = [IsStudent]

    def post(self, request):
        code = request.data.get('class_code', '').upper()
...
class TeacherClassViewAPI(RetrieveAPIView):
    permission_classes = [IsInstructor, IsClassroomInstructor]
    serializer_class = ClassroomStatsSerializer
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        for session in sessions[:10]:
            avg_focus = Performance.objects.filter(session=session).aggregate(
                Avg('focus_score')
            )['focus_score__avg'] or 0
            session_focus_data.append(round(avg_focus, 1))
            session_labels.append(session.start_time.strftime('%b %d'))
        
        # Reverse for chronological order
        session_focus_data.reverse()
        ...
        if last_session:
            attended_count = Performance.objects.filter(
                session=last_session, 
                attended=True
            ).count()
            total_students = classroom.enrollments.count()
            last_session_attendance = round((attended_count / total_students) * 100, 1) if total_students > 0 else 0
            
            last_session_avg_focus = Performance.objects.filter(
                session=last_session
            ).aggregate(Avg('focus_score'))['focus_score__avg'] or 0
            last_session_avg_focus = round(last_session_avg_focus, 1)
        ...
        stats = {
            ...
            'session_focus_data': session_focus_data,
...
class StudentClassViewAPI(RetrieveAPIView):
    permission_classes = [IsStudent, IsEnrolledStudent]
    serializer_class = StudentClassStatsSerializer
    queryset = Classroom.objects.all()
    lookup_field = 'pk'
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        for perf in performances[:10]:
            performance_data.append(perf.focus_score)
            performance_labels.append(perf.session.start_time.strftime('%b %d'))
        
        # Reverse for chronological order
        performance_data.reverse()
        performance_labels.reverse()
        
        last_perf = performances.first()
        
        stats = {
            'session_count': sessions.count(),
            'attended_count': performances.count(),
            'last_focus_score': last_perf.focus_score if last_perf else 0,
...
class SessionCreateAPI(CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        serializer.save(classroom=classroom, is_active=True)
...
class PerformanceCreateAPI(CreateAPIView):
    permission_classes = [IsInstructor]
    serializer_class = PerformanceSerializer

class EnrollmentListAPI(ListAPIView):
    permission_classes = [IsInstructor, IsClassroomInstructor]
